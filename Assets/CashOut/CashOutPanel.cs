...
public class CashOutPanel : GoldUIStain
{
    ...
    private void Start()
    {
        ...
        CloseBtn.onClick.AddListener(() => { FollyUIPeak(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = SapScanTip.instance.ShieldBulk.CashOut_MoneyName;
        InfoText.text = SapScanTip.instance.ShieldBulk.CashOut_Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = SapScanTip.instance.BaseLaw;
        ContactBtn.onClick.AddListener(() => { Application.OpenURL(SapScanTip.instance.BaseLaw); });
        ...
        {
            string url = SapScanTip.instance.BaseLaw + "/privacy_policy.html";
        ...
        PolicyToggle.isOn = MoreBulkUncover.TowWhen("CashOut_PolicyAgree");
    ...
    public override void Hidding()
    {
        ...
        FanwiseEnergyHatch.GetInstance().Cany(CShield.Raw_Mine_Sink_Reef, null);
        FanwiseEnergyHatch.GetInstance().Cany(CShield.uspanel, null);
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            PhotoUncover.GetInstance().PearPhoto("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            PhotoUncover.GetInstance().PearPhoto(AccountErrorText.text);
            return;
        }

        CashOutManager.GetInstance().Account = AccountInput.text;
        MoreBulkUncover.GunSmooth("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            PhotoUncover.GetInstance().PearPhoto("Please agree to the policy");
            return;
        }
        MoreBulkUncover.GunWhen("CashOut_PolicyAgree", true);
    ...
    public void UpdateMoney()
    {
        ...
        //进度条动画
        float MaxMoney = float.Parse(SapScanTip.instance.ShieldBulk.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            PhotoUncover.GetInstance().PearPhoto("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        PlowSenseGarden.GetInstance().CanySense("1301", LeftTimeText.text, CashOutManager.GetInstance().Money.ToString());
    }
}
