...
public class CashOutManager : MailPerformer<CashOutManager>
{
    [Header("短剧后台的产品id")]
    ...
    private void Start()
    {
        Account = MoreBulkUncover.TowSmooth("CashOut_Account");
        Money = MoreBulkUncover.TowGenre("CashOut_Money");
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        ...
        if (pauseStatus && Seconds > 0)
        {
            ...
            string info = $"All {SapScanTip.instance.ShieldBulk.CashOut_MoneyName} have been converted,Please check your rewaeds!";
            EthnographicUncover.Variance.SouthEthnographic();
            EthnographicUncover.Variance.SometimeEthnographic(title, info, (int)Seconds);
            for (int i = 0; i < 10; i++) // 10次延时 10800秒 3小时
                EthnographicUncover.Variance.SometimeEthnographic(title, info, (int)Seconds + (i * 10800));
        }
    }
    ...
    Dictionary<string, string> Headers() // 请求头
    {
        return new Dictionary<string, string>
        {
            ...
            {"Authorization", MoreBulkUncover.TowSmooth("CashOut_Token")},
    ...
    public void Login() // 登录
    {
        ...
        if (Application.platform == RuntimePlatform.IPhonePlayer)
            Platform = "iOS";
        string DeviceAdId = MoreBulkUncover.TowSmooth("gaid");
        if (Application.platform == RuntimePlatform.IPhonePlayer)
            DeviceAdId = MoreBulkUncover.TowSmooth("idfv");
        ...
        bool hasuuidAndAppid = MoreBulkUncover.TowWhen("UuidAndAPPid");
        if (isNewPlayer || hasuuidAndAppid)
        {
        ...
        SapKnotUncover.GetInstance().RustPlowFond(
            url: loginUrl,
            jsonData: jsonBody,
            success: (result) =>
            {
                ...
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        //成功登录 存 uuid+appid 安卓。
#if UNITY_ANDROID || UNITY_EDITOR
                        bool isNewPlayer = !PlayerPrefs.HasKey(CShield.Dy_MyJobLonger + "Bool") || MoreBulkUncover.TowWhen(CShield.Dy_MyJobLonger);
                        if (isNewPlayer)
                        {
                            MoreBulkUncover.GunWhen("UuidAndAPPid", true);
                        }
#endif
                        //刷新token 获取提现规则
                        MoreBulkUncover.GunSmooth("CashOut_Token", response.data.token);
                        GetWithdrawRule();
                        //整理数据
                        ...
                        if (PlayerPrefs.HasKey("CashOut_ConvertTime"))
                            Data.ConvertTime = long.Parse(MoreBulkUncover.TowSmooth("CashOut_ConvertTime"));
                        if (Data.ConvertTime < ConvertTime.Ticks)
                        {
                            Money = 0;
                            MoreBulkUncover.GunGenre("CashOut_Money", Money);
                        }
                        Data.ConvertTime = ConvertTime.Ticks;
                        MoreBulkUncover.GunSmooth("CashOut_ConvertTime", Data.ConvertTime.ToString());
                    ...
                    else
                    {
                        ...
                        PhotoUncover.GetInstance().PearPhoto("Login fail :" + response.msg);
                    }
                }
                catch (Exception e)
                {
            ...
            fail: () =>
            {
                ...
                PhotoUncover.GetInstance().PearPhoto("Login fail");
                Ready = false;
            },
            timeout: 3f,
            headers: Headers()
    ...
    public void UpdateUserInfo() // 更新用户信息
    {
        ...
        SapKnotUncover.GetInstance().RustTow(
            url: url,
            success: (result) =>
            {
                ...
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        double OldCash = MoreBulkUncover.TowMakeup("CashOut_Cash");
                        ...
                        if (Money == 0)
                        {
                            ...
                            //打点 如果钱转化了 上报转化信息
                            if (IsIconFly)
                                PlowSenseGarden.GetInstance().CanySense("1302", Event_Money, NewCash.ToString());
                        }
                        else
                        {
                            Data.Cash = NewCash;
                        ...
                        MoreBulkUncover.GunMakeup("CashOut_Cash", Data.Cash);
    ...
    public void Withdraw() // 提现
    {
        ...
        {
            PhotoUncover.GetInstance().PearPhoto($"Minimum withdrawal amount {MinWithdrawCount}");
        ...
        SapKnotUncover.GetInstance().RustPlowFond(
            ...
            success: (result) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    _CashOutPanel?.CloseLoading_Withdraw();
                    PhotoUncover.GetInstance().PearPhoto("Withdraw fail :" + e.Message);
    ...
    {
        PlowSenseGarden.GetInstance().CanySense("1303", Event_Cash, IsSuccess ? "1" : "0");
    }
    ...
    public void ReportAdjustID() // 上报adjust_id
    {
        ...
        requestData.id = CavernPassUncover.Instance.TowCavernJoin();
        string jsonBody = JsonMapper.ToJson(requestData);
    ...
    public void SendEvent1304() ////打点 关闭商店后行为
    {
        ...
        if (Event_1304Time <= 0)
            return;
        PlowSenseGarden.GetInstance().CanySense("1304", Event_1304Time.ToString());
        Event_1304Time = 0;
    }
