...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<SapScanTip>()[0].BaseLaw != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
...
#endif
        if (Resources.FindObjectsOfTypeAll<ADUncover>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADUncover>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADUncover>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<SapScanTip>()[0].LadeLast != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADUncover>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADUncover>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADUncover>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<CavernPassUncover>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<CavernPassUncover>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<CavernPassUncover>();
            }
        }
        GameObject.Find("MainManager").GetComponent<CavernPassUncover>().BotanyID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<LullMeUncover>())
        {
            GameObject.Find("MainManager").AddComponent<LullMeUncover>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<LullMeUncover>().Relic = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<LullMeUncover>().Relic = info.PackageName;
#endif

        Resources.FindObjectsOfTypeAll<SapScanTip>()[0].LadeLast = info.GameCode;
        Resources.FindObjectsOfTypeAll<SapScanTip>()[0].BaseLaw = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<SapScanTip>()[0].GoldStageLaw = info.BaseUrl + CShield.StageLaw;
        Resources.FindObjectsOfTypeAll<SapScanTip>()[0].GoldShieldLaw = info.BaseUrl + CShield.ShieldLaw;
        Resources.FindObjectsOfTypeAll<SapScanTip>()[0].GoldSwayLaw = info.BaseUrl + CShield.SwayLaw;
        Resources.FindObjectsOfTypeAll<SapScanTip>()[0].GoldCavernLaw = info.BaseUrl + CShield.CavernLaw;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CShield.ShieldLaw + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CShield.ShieldLaw + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CShield.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
