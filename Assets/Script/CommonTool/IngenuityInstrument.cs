...
public class IngenuityInstrument : MonoBehaviour
{
    void Start()
    {
    ...
    public static void LordNo(GameObject IconImage, float IconMoveYStart, float IconMoveYFinal,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void EatPear(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void EatAfar(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void SenateSenate(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void SenateSenate(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        SenateSenate(startNum, endNum, delay, text, "", finish);
    }
    public static void SenateSenate(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + SenateErie.MakeupNoShy(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }

    public static void WeedyNo(float startNum, float endNum,int nowLevel,int fullCount , float delay, Image image,Text text, System.Action finish)
    {
    ...
    /// <param name="finish"></param>
    public static void SenateLordlyAwful(float startValue,float endValue,Image sliderImage, System.Action finish, float delay = 0)
    {
    ...
    /// <param name="finish"></param>
    public static void DeftPear(GameObject HomeTopBarShow, float MoveOffsetShow, System.Action finish) 
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    /// <param name="MoveOffset"></param>
    /// <param name="finish"></param>
    public static void DeftAfar(GameObject HomeTopBarHide, float MoveOffsetHide, System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void RimeMust(GameObject GoldImage,Transform parent, int a ,Transform GoldStart,Transform GoldFinal,System.Action finish) 
    {
        if (a == 0)
        {
            finish();
    ...
    public static void RimeMustAcid(GameObject GoldImage, int a, Transform StartTF, Transform EndTF, System.Action finish)
    {
        //如果没有就算了
        if (a == 0)
        {
        ...
        for (int i = 0; i < a; i++)
        {
            ...
            s.Append(GoldIcon.transform.DOMove(new Vector3(GoldIcon.transform.position.x + OffsetX, GoldIcon.transform.position.y + OffsetY, GoldIcon.transform.position.z), 0.15f).SetDelay(Delaytime).OnComplete(() =>
            {
                ...
                if (t % 5 == 0)
                {
                    BrownTip.GetInstance().TossClutch(BrownOnce.UIMusic.Sound_GoldCoin);
                }
            }));
            //金币移动到最终位置
    ...
    /// <param name="i"></param>
    public static void FindDustMust(GameObject GameBankCashImage,Transform parent,Transform BankCashStart,Transform BankCashFinal,int i,System.Action finish)
    {
        ...
        if (i <= 2)
        {
            ...
            GameBankCash.transform.DOMoveY(BankCashFinal.position.y,0.7f).OnComplete(()=>{
                ...
                finish();
                //BrownTip.GetInstance().PlayEffect(BrownOnce.SceneMusic.Sound_GameBankGold);
            });
        }
        else if( i==3 )
        {
            ...
            GameBankCash.transform.DOMoveY(BankCashFinal.position.y, 0.7f).OnComplete(() => 
            {
                ...
                //BrownTip.GetInstance().PlayEffect(BrownOnce.SceneMusic.Sound_BankCashFull);
    ...
    /// <param name="finish"></param>
    public static void StarIceMust(GameObject StarBox,float BoxMoveOffset,GameObject Fx_StarBoxOpen, System.Action RewardShow) 
    {
    ...
    /// <param name="finish"></param>
    public static void TeemIceSecretPear(List<GameObject> CardList, int Count, System.Action finish)
    {
        ...
        float screenW = UIManager.GetInstance().CropStripe.GetComponent<CanvasScaler>().referenceResolution.x;
        int finishCount = Count;
        if (Count == 3) 
        {
            float Space1 = 80f;
            float Space2 = ((UIManager.GetInstance().CropStripe.GetComponent<CanvasScaler>().referenceResolution.x - Space1 * 2) - CardW * Count) / (Count - 1);
    ...
    /// <param name="finish"></param>
    public static void WeedyPicnicISexLipid(GameObject UnLevelIcon, Sprite LevelIcon, Text text, System.Action finish)
    {
    ...
    /// <param name="finish"></param>
    public static void AriseFlairAni(GameObject box,GameObject Fx_RewardBG,System.Action finish) 
    {
    ...
    /// <param name="i"></param>
    public static void Ratchet(GameObject BankBtn,int i) 
    {
        float offset = -0.6f;
    ...
    public static void LordLift(GameObject IconImage)
    {
    ...
    public static void OVOMust(GameObject GoldImage, Transform parent, int a, Transform GoldStart, Transform GoldFinal, System.Action NextShow, System.Action finish)
    {
        if (a == 0)
        {
            finish();
    ...
    /// <param name="Finish"></param>
    public static void ImpossibleChoice(GameObject obj, float addPosition, System.Action Finish)
    {
    ...
    public static void LordIdle(GameObject Less)
    {
        Less.transform.DOScale(0.52f, 0.08f).OnComplete(() => 
        {
            Less.transform.DOScale(0.7f, 0.08f).OnComplete(()=> 
            {
                Less.transform.DOScale(0.52f, 0.08f);
            }); 
        });
    }
    
    public static void DiverseDry(Vector3 target, GameObject Less, Vector3 Start, Transform parent,
        System.Action finish)
    {
        ...
        {
            Delaytime += 0.1f;
            GameObject GoldItem = Instantiate(Less, parent);
    ...
    /// fever模式动画
    /// </summary>
    IEnumerable  CloseAstronomy(List<GameObject> Lightlist) 
    {
    ...
    public static void DiverseSecretDry(Vector3 target, GameObject Less, Vector3 Start, Transform parent,
        System.Action finish)
    {
        GameObject GoldItem = Instantiate(Less, parent);
