...
{
    public static void Swear<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class TowMatterBulk : MailPerformer<TowMatterBulk>
{
    // Start is called before the first frame update
    public bool BabyIntend;
    public bool AxLoop;

    static public TowMatterBulk instance;
    ...
    /// <returns></returns>
    public Vector3 WhyAwaitTrace(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject TowFresherLadeWander(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float TraceNoVague(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Vague180No360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int TowGrazeCelebrate(Vector2 p1, Vector2 p2)
    {
        float angle = Vague180No360(TraceNoVague(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 WhyRespectRoleFoil(Renderer Renderer)
    {
    ...
    public Vector2 WhyRespectRoleFoil(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 WhyTimelyFoil(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 WhyNoisySubtlety()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = WhyUprootQuina();
        float screenH = WhyUprootWinter();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int WhyGospelTwain(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] TowBalladDemocrat(int total, int count)
    {
    ...
    public Dictionary<string, object> SmoothNoConstitute(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string ConstituteNoSmooth(Dictionary<string, object> Info)
    {
    ...
    public int DayTossDome(string dateStart, DateTime dateEnd)
    {
    ...
    public int RayTossDome(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan TossDome(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 TowIceAtPort(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = TowIceAtPort(canvas, text, strFragmentIndex);
            Vector3 lastPos = TowIceAtPort(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = TowIceAtPort(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 TowIceAtPort(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float GrandeurByVary(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Show= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string AnemoneDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string FurnaceDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Show;
